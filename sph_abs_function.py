...
if __name__ == "__main__":
    thetaE = 0
    dens = 19.32
    mass_atten_Ma = pow(0.98*10,3)
    mass_atten_La = pow(1.3*10,2)
    ...
    for y in range(0, 255):
        for x in range(0, 255):
            if (x-128)**2 + (y-128)**2 <= (radius*10**7)**2:
                img[y,x] = sph_ACF(radius, dens, mass_atten_Ma, mass_atten_La, thetaE, (x-128)*10**-7, (y-128)*10**-7,det1=True,det3=True)
